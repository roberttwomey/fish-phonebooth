...
class VisionSystem:
	def __init__(self, args):
		...
		self.minBlobSize = args.minblob if hasattr(args, 'minblob') else 600.0
		self.maxBlobSize = 50000.0
		self.searchRadius = args.radius if hasattr(args, 'radius') else 70.0
	...
	def update(self, time_left=-1):
		...
		blurred_frame = cv2.GaussianBlur(maskedframe, (5, 5), 0)

...
class PhoneBooth(): 
	...
	def storeNum(self):
		...
		if self.numberState == NEW:
			...
			print("added new number", self.listObj[self.uid])
	...
	try:
		while(1):
			...
			if booth.boothState == RUNNING_PROGRAM:
				# display cameras with countdown
				vision.update(DURATION-(time.time()-booth.timeStarted))
			else:
				vision.update(-1)
